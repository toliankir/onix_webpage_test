...
export default class NeededComponent extends React.Component {
    list = ["template1","template2", "template3", "template4"]
    ...
    render() {
        ...
        <table>
            <thead>
                <tr>
                    { [...Array(4)].map((_, index) => <th key={`th_${index}`}>{this.getRandomWordFromLorem()}</th>) }
                </tr>
            </thead>
            <tbody>
                { [...Array(6)].map((_, index) => {
                return <tr key={`tr_${index}`}>
                    { [...Array(4)].map((_, index) => <td key={`td_${index}`}>{this.getRandomWordFromLorem()}</td>) }
                </tr>
                }) }
            </tbody>
        </table>
